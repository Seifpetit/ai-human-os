...
The canonical (machine-readable) source of these definitions is `AI-Human OS/memory/CANONICAL_DEFINITIONS.json`.

- Contract: An artifact plus the invariants its consumer is allowed to assume. If invariants are not satisfied, the consumer must block, retry, or fail explicitly.
- Contract layer: A phase boundary that produces and validates a contract artifact before downstream work may proceed.
- Cycle: One planned `operation_key` applied to one `file_path` across zero or more retries. Gate 3 can pause a cycle after verification; the cycle is not complete until the verified artifact is approved (or auto-approved) and committed.
- Run: One invocation of `node run_ai.js` that attempts to apply zero or more cycles against the current compiled plan and state. A run may have zero cycles (preflight/gate failures).
- Complete run: A run whose canonical summary reports `lifecycle.status=completed`, `failure.classification=none`, and the operator reached `IMPLEMENTATION COMPLETE` (no remaining operations) for the compiled plan.
